...
def _adc_spi_transform(address, field, channel=0):
    ...
    _logger.debug(
        "_adc_spi_transform:channel={:} {:} -> {:}".format(
            channel, _map_str(field, address), _map_str(tfield, taddress)
        )
    )
...
def _channel_transform(address, field, channel=0):
    ...
    _logger.debug(
        "_channel_transform:channel={:} {:} -> {:}".format(
            channel, _map_str(field, address), _map_str(tfield, taddress)
        )
    )
    return taddress, tfield
...
class _RegisterMap:
    ...
    def __set__(self, instance, values):
        ...
        if isinstance(values, dict):  # dictionary assignment to map
            _logger.debug("_RegisterMap.__set__({}) to {}".format(self.name, values))
...
class _RegisterGroup(_RegisterMap):
    ...
    def __setattr__(self, attr, value):
        ...
        _logger.debug("RG:({}).__setattr__:{} to {}".format(rg_name, attr, value))
        if attr in self.reg_map:
            ...
            if isinstance(value, dict):
                if self.reg_map[attr].loadable:
                    ...
                    _logger.debug(
                        "RG:({}).__setattr__:{} not loadable".format(rg_name, attr)
                    )
                    return
            self.reg_map[attr].set(
            ...
            for reg, value in group_values.items():
                if reg in self.reg_map:
                    ...
                    if self.reg_map[reg].loadable:
                        ...
                        _logger.debug(
                            "RG:{} __set__ {} is not loadable".format(self.name, reg)
                        )
                else:
    ...
    def _set_indices(self, index):
        ...
        _logger.debug(
            "RegisterGroup:{}._set_indices {} -> {}".format(self.name, index, indices)
        )
    ...
    def __setitem__(self, index, value):
        ...
        if isinstance(value, dict):  # dictionary assignment to entire group
            _logger.debug("RG:{}.__setitem__({}) to {}".format(self.name, index, value))
    ...
    def __setattr__(self, key, value):
        if key == "all":
            ...
            if not isinstance(value, dict):
                raise AttributeError("dict required, not {}".format(type(value)))
            for group in value:
            ...
            return
        _logger.debug("Registers.__setattr__({}) -> super().__setattr__".format(key))
